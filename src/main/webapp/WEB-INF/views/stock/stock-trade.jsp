...
<body>
    ...
    <div class="container stock-trade-container mt-5">
        ...
        <div class="stock">
            ...
            <div style="display: flex; justify-content: space-between">
                <h2 id="realtime-price">
                	<fmt:formatNumber value="${ stockPreview.price }" pattern="#,##0" />원
                </h2>
                <c:choose>
                    <c:when test="${ stockPreview.dailyPriceChangeRoundedPercent>0 }">
                        <h2  id="daily-change-rate" style="color: red">▲${ Math.abs(stockPreview.dailyPriceChangeRoundedPercent) }%</h2>
                    </c:when>
                    <c:when test="${ stockPreview.dailyPriceChangeRoundedPercent<0 }">
                        <h2  id="daily-change-rate" style="color: blue">▼${ Math.abs(stockPreview.dailyPriceChangeRoundedPercent) }%</h2>
                    </c:when>
                    <c:otherwise>
                        <h2 id="daily-change-rate">${ stockPreview.dailyPriceChangeRoundedPercent }%</h2>
                    </c:otherwise>                    
                </c:choose>                
                <h2 id="prev-day-close-price">
                	<fmt:formatNumber value="${ stockPreview.prevDayClosePrice }" pattern="#,##0" />원
                </h2>
            </div>
        </div>
    ...
    <c:if test="${ not empty tradeResultMsg }">
	    <script>
	    	alert("${tradeResultMsg}");
	    </script>
    </c:if>
